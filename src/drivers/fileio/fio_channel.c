...
static camio_error_t fio_read_peek( camio_channel_t* this)
{
    //This is not a public function, can assume that preconditions have been checked.
    //DBG("Doing read peek\n");
    fio_channel_priv_t* priv = STREAM_GET_PRIVATE(this);
...
static camio_error_t fio_read_ready(camio_muxable_t* this)
{
    ...
    fio_channel_priv_t* priv = STREAM_GET_PRIVATE(this->parent.channel);
    ...
    camio_error_t err = fio_read_peek(this->parent.channel);
...
static camio_error_t fio_read_request( camio_channel_t* this, camio_read_req_t* req_vec, ch_word req_vec_len )
{
...
static camio_error_t fio_read_acquire( camio_channel_t* this,  camio_rd_buffer_t** buffer_o)
{
...
static camio_error_t fio_read_release(camio_channel_t* this, camio_rd_buffer_t** buffer)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
static camio_error_t fio_write_acquire(camio_channel_t* this, camio_wr_buffer_t** buffer_o)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
static camio_error_t fio_write_request(camio_channel_t* this, camio_write_req_t* req_vec, ch_word req_vec_len)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This is null???\n"); //WTF?
...
static camio_error_t fio_write_try(camio_channel_t* this)
{
    fio_channel_priv_t* priv = STREAM_GET_PRIVATE(this);
...
static camio_error_t fio_write_ready(camio_muxable_t* this)
{
    ...
    //OK now the fun begins
    camio_error_t err = fio_write_try(this->parent.channel);
    if(err == CAMIO_ENOERROR){
...
static camio_error_t fio_write_release(camio_channel_t* this, camio_wr_buffer_t** buffer)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
static void fio_destroy(camio_channel_t* this)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
camio_error_t fio_channel_construct(camio_channel_t* this, camio_controller_t* controller, fio_params_t* params, int rd_fd, int wr_fd)
{
    ...
    DBG("Done constructing FIO channel with read_fd=%i and write_fd=%i\n", priv->rd_fd, priv->wr_fd);
    return CAMIO_ENOERROR;
}

NEW_STREAM_DEFINE(fio,fio_channel_priv_t)
