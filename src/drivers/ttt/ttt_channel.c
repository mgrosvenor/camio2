...
static camio_error_t ttt_read_peek( camio_channel_t* this)
{
    ...
    DBG("Got %lli bytes from TTT peek\n", priv->rd_buffer->data_len );
...
static camio_error_t ttt_read_ready(camio_muxable_t* this)
{
    ...
    camio_error_t err = ttt_read_peek(this->parent.channel);
...
static camio_error_t ttt_read_request( camio_channel_t* this, camio_rd_req_t* req_vec, ch_word req_vec_len )
{
    ...
    if( req_vec->dst_offset_hint > CAMIO_TTT_BUFFER_SIZE){
        DBG("You're trying to set a buffer offset (%i) which is greater than the buffer size (%i) ???\n",
                req_vec->dst_offset_hint, CAMIO_TTT_BUFFER_SIZE); //WTF?
        return CAMIO_EINVALID;
    }
    ...
    if(req_vec->read_size_hint > 0){
        if(req_vec->read_size_hint > CAMIO_TTT_BUFFER_SIZE){
            //TODO XXX: Could realloc buffer here to be the right size...
            req_vec->read_size_hint = CAMIO_TTT_BUFFER_SIZE;
        }
    }
...
static camio_error_t ttt_read_acquire( camio_channel_t* this,  camio_rd_buffer_t** buffer_o)
{
...
static camio_error_t ttt_read_release(camio_channel_t* this, camio_rd_buffer_t** buffer)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
static camio_error_t ttt_write_acquire(camio_channel_t* this, camio_wr_buffer_t** buffer_o)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
static camio_error_t ttt_write_ready(camio_muxable_t* this)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This is null???\n"); //WTF?
    ...
    //OK now the fun begins
    camio_error_t err = ttt_write_try(this->parent.channel);
    if(err == CAMIO_ENOERROR){
...
static camio_error_t ttt_write_release(camio_channel_t* this, camio_wr_buffer_t** buffer)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
static void ttt_destroy(camio_channel_t* this)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
camio_error_t ttt_channel_construct(camio_channel_t* this, camio_controller_t* controller, int fd)
{
    ...
    error = buffer_malloc_linear_new(this,CAMIO_TTT_BUFFER_SIZE,CAMIO_TTT_BUFFER_COUNT,true,&priv->rd_buff_pool);
    if(error){
        DBG("No memory for linear read buffer!\n");
        return CAMIO_ENOMEM;
    }
    error = buffer_malloc_linear_new(this,CAMIO_TTT_BUFFER_SIZE,CAMIO_TTT_BUFFER_COUNT,false,&priv->wr_buff_pool);
    if(error){
    ...
    this->wr_muxable.vtable.ready      = ttt_write_ready;
    this->wr_muxable.fd                = priv->fd;


    DBG("Done constructing TTT channel with read_fd=%i and write_fd=%i\n", fd, fd);
    return CAMIO_ENOERROR;
}
