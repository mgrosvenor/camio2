...
static camio_error_t ttt_read_request( camio_stream_t* this, camio_read_req_t* req_vec, ch_word req_vec_len )
{
...
static camio_error_t ttt_write_request(camio_stream_t* this, camio_write_req_t* req_vec, ch_word req_vec_len)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This is null???\n"); //WTF?
...
static camio_error_t ttt_write_try(camio_stream_t* this)
{
    ttt_stream_priv_t* priv = STREAM_GET_PRIVATE(this);
...
static camio_error_t ttt_write_ready(camio_muxable_t* this)
{
    ...
    //OK now the fun begins
    camio_error_t err = ttt_write_try(this->parent.stream);
    if(err == CAMIO_ENOERROR){
...
static camio_error_t ttt_write_release(camio_stream_t* this, camio_wr_buffer_t** buffer)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
...
camio_error_t ttt_stream_construct(camio_stream_t* this, camio_controller_t* connector, int fd)
{
    //Basic sanity checks -- TODO XXX: Should these be made into (compile time optional?) asserts for runtime performance?
    if( NULL == this){
        DBG("This null???\n"); //WTF?
    ...
    DBG("Done constructing TTT stream with read_fd=%i and write_fd=%i\n", fd, fd);
    return CAMIO_ENOERROR;
}
